    ...
    private
      ...
      def set_for_next_retry
        if failed_attempts < 3
          ...
          self.next_occurrence_at = self.next_occurrence_at + 7.days
          self.save
        ...
        if parent_order.promo_total.abs > 0
          if parent_order.line_items.count == 1
            ...
            if promo_line_items.blank?
              ...
              parent_line_items = parent_order.line_items
              promo_line_item = promo_line_items.select{|s| parent_line_items.map(&:variant_id).include?(s.variant_id)}.first
